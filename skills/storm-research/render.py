...
def emit_dossier(data, out_path):
    """After the briefing HTML ships, persist the source JSON sidecar + upsert the
    combined Arc dossier (artifacts/research/<slug>.dossier.json). Best-effort:
    a failure here must NEVER break the briefing render, so it is fully guarded."""
    try:
        out_abs = os.path.abspath(out_path)
        base = os.path.splitext(out_abs)[0]            # .../<slug>  (briefing is "<slug>.html")
        sidecar = base + ".json"
        ...
        if os.path.exists(arc):
            ...
            subprocess.run([sys.executable, arc, "--slug", slug, "--date", date,
                            "--briefing", sidecar], check=False)
    except Exception as e:
        print(f"render.py: dossier upsert skipped ({e})", file=sys.stderr)
