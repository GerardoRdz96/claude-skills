...
**North-Star trend metric — the one-window KPI (reported beside the Cadence score, not scored).** "What percentage of my work can I do from this window right here?" — the AIOS-maturity number (Nate Herk, 2026-07-11 6h course; doctrine in `references/four-cs.md` § Cadence). Measure it when the project ships the estimator (Servy does): `python3 scripts/skill-harvest.py --share --days 14` prints the two honest proxies from real transcripts — skill-mediated share of in-Servy work + unattended-session share. Transcripts can't see work done OUTSIDE the window, so also ask the user for their subjective % and record both. Track the trio across audits; a rising trend is the compounding hook made visible.

### Step 2.5: Hygiene checks (advisory — always reported, never scored)

Two checks added 2026-07-11 (Nate 6h course). They ride project-local helpers — run what's present, skip silently what isn't. Advisory findings ARE eligible for the Top 3 Gaps list (Step 3): rank a hygiene finding as if it lost 3 points, key-hygiene flags at 1.5× (keys-not-prompts is security, not cosmetics).

**a) Context hygiene.** Grade the setup against `references/claude-code-context-management.md` § "The discipline checklist" (the page names `/audit` as its grader). Check the statically checkable items and report pass/fail: CLAUDE.md byte budget (`wc -c CLAUDE.md` — target <16 KB, hard cap 24 KB), statusline configured (`statusLine` in `.claude/settings.json`), `.claudeignore` where bulky folders exist, MCP surface lean (~18k tokens per server per message), noisy-command denies where the project warrants them. Then the **fresh-session baseline**: Claude can't run `/context` itself, so ask the user for the `/context` figure from a fresh session and compare against the recorded datapoints on that wiki page (~30-40k blank floor / Nate's 51k / older 62k). Offer to log the figure as a dated datapoint on that page — consent-gated, since it's a write beyond the report.

**b) Key hygiene.** Run `python3 scripts/key-hygiene.py` (read-only; prints key NAMES only, never values). It inventories env-file key names against `connections.md` and flags per the ElevenLabs scoped-key checklist in `references/four-cs.md` (name-per-purpose · credit-cap · endpoint-cap): `unregistered`, `not-named-per-purpose`, `no-cap-noted`, `no-scope-noted`. Report the counts; the cap/scope flags are proxies (annotations, not provider state) — the fix is provider-side scoping, then annotate the env line or the connections.md row.
...
One-window KPI: {skill-mediated}% skill-mediated · {unattended}% unattended · user's own estimate {pct or "not asked"} ({trend vs last audit, if a prior report exists})

## Hygiene (advisory)
- Context: {passed}/{checked} discipline-checklist items pass; fresh-session baseline {tokens, or "ask the user for /context"}
- Keys: {flagged}/{total} keys flagged ({unregistered} unregistered · {no-cap} no-cap-noted · {no-scope} no-scope-noted) — names only

(Omit whichever hygiene line has no helper/data in this project.)
...
After printing, ask: "Save this audit to `audits/audit-{date}.md` so you can track score over time?" If yes, write it (creating `audits/` folder if needed). The only other writable side effect is the consent-gated `/context` datapoint from Step 2.5a.

## Notes

- **Read-only by default.** Never modify CLAUDE.md, memory, skills, or any project files. Optional writes are exactly two, both consent-gated: the audit report, and the fresh-session `/context` datapoint on the context-management wiki page (Step 2.5a).
